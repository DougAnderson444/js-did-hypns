...
export const createId = (did, fragment, separator, options) => {
  ...
  if (
    ...
    Object.values(SEPARATORS).some((sep) => prefix.includes(sep))
  ) {
...
export const assertDocument = (content) => {
  ...
  if (!context) {
    throw new InvalidDocument(
      'Document content must contain "@context" property.'
    )
  } else if (Array.isArray(context)) {
    if (context[0] !== DEFAULT_CONTEXT) {
      throw new InvalidDocument(
        `First "@context" value must be: "${DEFAULT_CONTEXT}". Found: "${context[0]}"`
      )
    }
  } else if (isString(context)) {
    if (context !== DEFAULT_CONTEXT) {
      throw new InvalidDocument(
        `Document with only one "@context" value must be none other than: "${DEFAULT_CONTEXT}". Found: "${context}"`
      )
    }
  } else {
    throw new InvalidDocument(
      'Document "@context" value must be a string or an ordered set.'
    )
  }
  ...
  if (!id) {
    ...
    throw new InvalidDocument(
      `Document "id" must be a valid DID. Found: "${id}"`
    )
  }
}
